...
public class Collider implements IPostEntityProcessingService {
        ...
        for (Entity entity : world.getEntities()) {
            for (Entity collisionDetection : world.getEntities()) {
                ...
                if (this.Collides(entity, collisionDetection)) {
                    // if dk.sdu.mmmi.cbse.bulletsystem.entity has been hit, and should have its life reduced
                    if (entityLife.getLife() > 0) {
                        entityLife.setLife(entityLife.getLife() - 1);
                        // if dk.sdu.mmmi.cbse.bulletsystem.entity is out of life - remove
                        if (entityLife.getLife() <= 0) {
                            world.removeEntity(entity);
                        }
                    }
    ...
    public Boolean Collides(Entity entity, Entity entity2) {
        ...
        if (distance < (entity.getRadius() + entity2.getRadius())) {
            //To ensure Enemy ships and Asteroids do not collide
            return (!(entity instanceof Enemy) || !(entity2 instanceof Asteroid))
                    && (!(entity instanceof Asteroid) || !(entity2 instanceof Enemy))
                    && (!(entity instanceof Player) || !(entity2 instanceof Bullet))
                    && (!(entity instanceof Bullet) || !(entity2 instanceof Player));
        }
        return false;
    }
